...
class Blyss(object):
    ...
    def __init__(self, device, knownDevices):
        ...
        self.logger.debug(f"{device.name}: Starting Blyss device ({subType}) @ {devAddress}")
        ...
        self.logger.info(f"Configured Blyss device '{device.name}' ({device.id}) @ {address}")
    ...
    def handler(self, frameData, knownDevices):
        ...
        devAddress = "BLYSS-" + frameData['infos']['id']
        self.logger.threaddebug(f"Blyss frame received: {devAddress}")
